...
public class ColorPickerPreference extends Preference
  ...
  protected void onSetInitialValue(boolean restorePersistedValue, Object defaultValue) {
    // when using PreferenceDataStore, restorePersistedValue is always true (see Preference class
    // for reference)
    // so we load the persistent value with getPersistedInt if available in the data store,
    // and use defaultValue as fallback (onGetDefaultValue has been already called and it loaded the
    // android:defaultValue attr from our xml).
  ...
  /**
   * Restore a default value, not necessarily a color For example: Set default value to -1 to remove
   * a color filter
   *
  ...
  private void setPreviewColor() {
    ...
    final int imageColor =
        ((mCurrentValue & 0xF0F0F0) == 0xF0F0F0) ? (mCurrentValue - 0x101010) : mCurrentValue;
