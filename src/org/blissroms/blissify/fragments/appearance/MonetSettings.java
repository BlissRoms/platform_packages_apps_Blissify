...
package org.blissroms.blissify.fragments.appearance;
...
import org.blissroms.blissify.preferences.colorpicker.ColorPickerPreference;
import org.json.JSONException;
import org.json.JSONObject;

@SearchIndexable
public class MonetSettings extends BlissifyDashboardFragment implements OnPreferenceChangeListener {
  ...
  private void updatePreferences() {
    final String overlayPackageJson =
    ...
    if (overlayPackageJson != null && !overlayPackageJson.isEmpty()) {
      try {
        ...
        if (!styleUpdated) {
          updateListByValue(mThemeStylePref, mThemeStylePref.getEntryValues()[0].toString());
        }
        // color handling
        final String sourceVal =
            (source == null || source.isEmpty() || (source.equals(COLOR_SOURCE_HOME) && both))
                ? "both"
                : source;
  ...
  private JSONObject getSettingsJson() throws JSONException {
    final String overlayPackageJson =
        Settings.Secure.getStringForUser(
            ...
            UserHandle.USER_CURRENT);
    JSONObject object;
    if (overlayPackageJson == null || overlayPackageJson.isEmpty()) return new JSONObject();
